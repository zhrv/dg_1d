...
int main(int argc, char** argv) 
{
	...
	while (t < TMAX) {
		...
		calcIntegral();			// вычисл€ем интеграл от(dF / dU)*deltaU*dFi / dx
		calcMatrWithTau();		// вычисл€ем матрицы перед производной по времени
		calcMatrFlux();			// ¬ычисл€ем потоковые величины 
		calcRHS();				// ¬ычисл€ем столбец правых членов
	...
	/* принудительна€ запись последнего шага  */
	saveCSV(step); 
...
void init() {
	...
	// узлы квадратур √аусса и центры €чеек
	for (int i = 0; i < N; i++){
		double x1 = XMIN + i*h;
		double x2 = x1 + h;
...
void calcMatrFlux() { 
	...
	for (int iCell = 1; iCell < N - 1; iCell++) {
		...
		{ //!< вычисл€ем потоки на границе iCell+1/2
...
void calcSmoother() {
	...
	for (int idField = 0; idField < 3; idField++) {
		...
		for (int iCell = 0; iCell < N-1; iCell++) {
			...
			fInt1 = 0.;
			fInt2 = 0.;
			for (int iGP = 0; iGP < 2; ++iGP) // по точкам √аусса
			{
